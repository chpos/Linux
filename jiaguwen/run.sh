...
function Start {
    echo -e " [Intro] warp-google"
    echo -e " [Intro] Test System:Debian 10"
    echo -e " [Intro] Version:2021-10-26-12"
...
function Download_Profile {
    wget -qO /etc/dnsmasq.d/warp.conf https://okr.pub/sh/warp-google/dnsmasq/warp.conf
    wget -qO /etc/wireguard/up https://okr.pub/sh/warp-google/scripts/up
    wget -qO /etc/wireguard/down https://okr.pub/sh/warp-google/scripts/down
...
function Generate_WireGuard_WARP_Profile {
    ...
    wget -qO /etc/wireguard/wgcf https://github.com/ViRb3/wgcf/releases/download/v2.2.9/wgcf_2.2.9_linux_amd64
    ...
    sed -i "s/1280/1420/g" /etc/wireguard/wg.conf
